...
This module configures the pytest environment for the `Pisces` testing suite. It provides custom
...
1. **Modify the `test_configuration.yaml` File**:
   ...
   .. code-block:: yaml
       ...
       REMOTE:
         HOST: "https://astro.utah.edu/~u1281896/Pisces"

       SETUP:
         RUN_BEFORE_ALL:
           - ["Pisces.tests.test_model", "test_model_build"]
...
3. **Package Results for Distribution**:
   ...
   the operating system, Python version, and ``Pisces`` version.
"""
import os
...
from pisces.tests.utils.environment import load_config
from pisces.utils import cgparams
...
def packager(request: pytest.FixtureRequest) -> Generator[None, None, None]:
    ...
    **Output Management**: To ensure clean output in environments like CI/CD systems, the ``capmanager`` plugin
    ...
    based on the operating system, Python version, and ``Pisces`` version.
    ...
    if not _answer_store:
        ...
        print(
            "\n[Pisces tests]: Skipping --package directive because --answer_store was False."
        )
    ...
    with capmanager.global_and_fixture_disabled():
        print("\0337", end="", flush=True)
        print(
            f"\n[Pisces tests]: Packaged answers for distribution: {package_name}.",
            end="\n",
        )
        print("\0338", end="", flush=True)
